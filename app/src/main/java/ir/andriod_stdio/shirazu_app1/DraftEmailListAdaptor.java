...
public class DraftEmailListAdaptor extends ArrayAdapter<EmailsRow> {

    private Context activity;
    private List<EmailsRow> friendList;
    int mResource;

    public DraftEmailListAdaptor(Context context, int resource, List<EmailsRow> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.friendsUsername.setText("Draft");
        holder.friendsUsername.setTextColor(Color.parseColor("#ffcc0000"));


        /*inja  ham mishe avalin  harfe sme kasi k email bara oone ro bezarim too circle
        ham user name kasi k mikhaiim behesh email bedim va age chenin fardi babood be ja oon D bezarim!
        ino ba'd doros kon (fateme):)*/

        //get first letter of each String item
        String firstLetter;
        firstLetter=" ";
        if(getItem(position).getUserName()!=null){
            firstLetter = String.valueOf(getItem(position).getUserName().charAt(0));
        }
